...
export default function App() {
  ...
  const [dedupeDirection, setDedupeDirection] = useState<"first" | "last">("first");
  ...
  const fetchLinks = useCallback(async () => {
    setSyncing(true);
    try {
      const data = await api<{ list: Link[] }>("/links");
  ...
  function handleDedupe() {
    ...
    const scope = isFilterActive ? filteredLinks : allLinks.filter((l) => !hiddenIds.has(l.id));
    ...
    for (const arr of groupsMap.values()) {
      ...
      for (let i = 1; i < arr.length; i++) {
        ...
        else if (s === ks && dedupeDirection === "last") keeper = arr[i];
      }
    ...
    askConfirm({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          await api(`/links/bulk`, { method: "DELETE", body: JSON.stringify({ ids }) });
  ...
  async function handleAdd() {
    ...
    setSyncing(true);
    try {
      const created = await api<Link>("/links", { method: "POST", body: JSON.stringify({ title: t, magnet: addMagnet.trim() }) });
  ...
  async function handlePasteAdd() {
    ...
    try {
      ...
      await api("/links/bulk", { method: "POST", body: JSON.stringify(payload) });
      await fetchLinks();
      setAddMagnet("");
      ...
      <div className="mv-app">
        ...
        <header className="mv-header">
          ...
          <div>
            ...
            <p className="mv-subtitle">Neon · Torrent Link Manager</p>
          </div>
        </header>
        ...
        <details className="mv-collapse">
          ...
          <div className="collapsible-body">
            <p className="hint">When duplicate episodes or seasons are found, the copy whose link contains the highest-ranking priority word is kept. Use arrows to rank.</p>
            ...
            <div className="filter-row" style={{ marginTop: "0.75rem" }}>
              <div className="input-wrap" style={{ flex: 1 }}>
                <select value={dedupeDirection} onChange={(e) => setDedupeDirection(e.target.value as any)}>
                  <option value="first">Tie-breaker: Keep First in List</option>
                  <option value="last">Tie-breaker: Keep Last in List</option>
                </select>
              </div>
              <div className="input-wrap" style={{ flex: 1 }}>
                <input type="text" className="no-icon" value={dedupeIgnore} onChange={(e) => setDedupeIgnore(e.target.value)} placeholder="Phrase to ignore (e.g. CAM)" />
              </div>
            </div>
          </div>
        </details>
